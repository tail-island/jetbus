...
Kotlinがこのような離れ業をするには変数や関数の戻り値が`null`になりえるかどうかをコード上で表現できなければならないわけで、Kotlinでは、それを型名の後ろに`?`がつくかどうかで表現しています。`Int`なら`null`になることはない、`Int?`ならば`null`になる可能性があるって感じ。上のコードのRetrofit2の`Response`の`body()`は`JsonArray?`を返すので、その戻り値のメソッドを呼ぶとコンパイル・エラーになるというわけ。
